...
def parse_args():
    if len(sys.argv) != 6:
        print("Usage:", sys.argv[0], "[path to laser] [path to command] [path to dynamic map] [path to trajectories] [path to true command]")
        exit(-1)
    return sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]

laser_file, command_file, dynamic_map_file, trajectories_file, true_command_file = parse_args()
...
for i in range(laser_data.shape[0]):
    ...
    plt.scatter(l[:, 0], l[:, 1], s=0.1, color='green')
    draw_trajectory(t)
